...
This script automates batch submission of [pytom-match-pick](https://github.com/SBC-Utrecht/pytom-match-pick) jobs on an HPC cluster (SLURM) by reading metadata directly from **[Warp](http://warpem.com/) / WarpTools** tilt-series XMLs. It:

- Extracts **tilt angles** from `<Angles>` in each `Position_*.xml` (sign-flipped to the PyTom convention)
- Reads **per-tilt defocus** (μm) from the `<GridCTF>` nodes (falls back to `<GridCTFDefocus>`)
- Builds **per-tilt exposure** from `<Dose>` if present, otherwise cumulatively sums `--dose` in acquisition order
- Locates the matching reconstruction in `reconstruction/` and generates SLURM submission scripts

and will run on all tilt-series matched by `--pattern` (default `Position*.xml`) unless restricted with `--include` / `--exclude`.
...
### Robust tomogram matching

Reconstructions are resolved per tilt-series by trying `reconstruction/<prefix>.mrc`, then `<prefix>_Vol.mrc`, then `<prefix>_*.mrc`. The last (suffix) case explicitly **excludes files that belong to another tilt-series**, using the full set of known prefixes — so `Position_1` is never confused with `Position_1_2`, nor `Position_6` with `Position_6_13`, nor `Position_11` with `Position_111`, regardless of numeric or non-numeric suffixes.
...
- A SLURM cluster with a `pytom-match-pick` environment module (`ml pytom-match-pick`)
- A Warp tilt-series directory containing `Position_*.xml` and a `reconstruction/` subfolder
...
./warp2pytom.py \
  ...
  --dose 3 \                     # optional fallback if XML has no <Dose> (e-/Å² per tilt)
...
### `--mode per-tomo`
One self-contained script per tomogram:
...
BMASK_DIR=""
PREFIX="Position_1"
OD="submission/Position_1"
TOMO="/path/to/warp/tiltseries/reconstruction/Position_1_Vol.mrc"
TLT="${OD}/${PREFIX}.tlt"
DF="${OD}/${PREFIX}_defocus.txt"
EXP="${OD}/${PREFIX}_exposure.txt"
if [[ -z "${TOMO}" || ! -f "${TOMO}" ]]; then echo "Tomogram not found for ${PREFIX}"; exit 2; fi
